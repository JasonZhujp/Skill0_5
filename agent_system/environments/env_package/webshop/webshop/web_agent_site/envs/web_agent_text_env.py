...
class SimServer:
    ...
    def __init__(
        ...
        # Seed before get_goals so that price_upper (random.sample inside
        # get_human_goals) is deterministic across workers and runs.
        random.seed(seed)
        self.goals = get_goals(self.all_products, self.product_prices, human_goals)

        # Re-seed before shuffle to decouple from get_goals' random consumption.
        random.seed(seed)
