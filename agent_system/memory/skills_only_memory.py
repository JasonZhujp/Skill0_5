    ...
    * ``"embedding"`` – the task description is encoded with a
    ...
    def __init__(
        ...
        Args:
            ...
            include_general:      Whether to include general_skills and
                                  common_mistakes. If False, only task-specific
                                  skills (via embedding retrieval) are returned.
        """
    ...
    def _embedding_retrieve(
        ...
        # Top-k general skills (0 means skip — caller handles full inclusion)
        if top_k_general > 0:
    ...
    def retrieve(
        ...
        **kwargs,
        ...
        # ----------------------------------------------------------------
        # Embedding mode: semantic ranking of task-specific skills only;
        # general skills and common mistakes are always fully included.
        # ----------------------------------------------------------------
        if self.retrieval_mode == "embedding":
            ts_top_k = self.task_specific_top_k if self.task_specific_top_k is not None else top_k
            general_skills = self.skills.get('general_skills', []) if self.include_general else []
            returned_mistakes = common_mistakes if self.include_general else []
            _, task_skills = self._embedding_retrieve(
                task_description=task_description,
                top_k_general=0,  # not used; general skills are fully included
            ...
            return {
                ...
                'mistakes_to_avoid': returned_mistakes,
                'task_type': task_type,
    ...
    def format_for_prompt(self, retrieved_memories: Dict[str, Any], exclude_hints: bool = False) -> str:
        """
        Format retrieved skills into a string suitable for prompt injection.

        Args:
            retrieved_memories: Dict returned by :meth:`retrieve`.
            exclude_hints: If True, only output task-specific skills,
                skipping general_skills and common_mistakes (the "hints"
                that guide-internalization wants to strip for the plain prompt).
        ...
        # 1) Task-specific skills (top-k retrieved, placed first for salience)
