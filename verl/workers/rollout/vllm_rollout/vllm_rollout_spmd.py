...
# ============================================================
# 确定性采样开关：开启后，每个请求的 seed = call_count * 100000 + batch_idx
# 保证同一 step 同一位置的请求跨 run 生成相同 response，同时组内不同位置有多样性。
# 通过环境变量控制: export VERL_DETERMINISTIC_SAMPLING=1 开启
# ============================================================
DETERMINISTIC_SAMPLING = os.getenv("VERL_DETERMINISTIC_SAMPLING", "0") == "1"
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self._generate_call_count = 0  # for DETERMINISTIC_SAMPLING seed
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            # Deterministic sampling: per-request seed for reproducibility across runs.
            # seed = call_count * 100000 + request_idx ensures:
            #   - same call_count + same position → same seed across runs
            #   - different positions within a batch → different seeds (diversity)
            if DETERMINISTIC_SAMPLING and do_sample:
                call_id = self._generate_call_count
                per_request_params = []
                for req_idx in range(batch_size):
                    sp = deepcopy(self.sampling_params)
                    sp.seed = call_id * 100000 + req_idx
                    per_request_params.append(sp)
                sampling_params_to_use = per_request_params
            else:
