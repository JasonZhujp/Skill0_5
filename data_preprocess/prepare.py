...
if __name__ == '__main__':
    ...
    **NOTE**: This is a frequently asked question.
    ...
    # These parquet files are placeholders (real tasks come from env), so
    # when the requested size exceeds the split, cycle indices to fill up.
    def _select_cyclic(ds, n):
        if n <= len(ds):
            return ds.select(range(n))
        idxs = list(range(len(ds))) * (n // len(ds)) + list(range(n % len(ds)))
        return ds.select(idxs)

    train_dataset = _select_cyclic(dataset['train'], args.train_data_size)
    test_dataset = _select_cyclic(dataset['test'], args.val_data_size)
